...
namespace Bluehost\Plugin\WP\McpSchema\Client\Elicitation\Factory;

use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\Union\EnumSchemaInterface;
use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\Factory\SingleSelectEnumSchemaFactory;
use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\Factory\MultiSelectEnumSchemaFactory;
use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\DTO\LegacyTitledEnumSchema;
/**
...
final class EnumSchemaFactory
{
    ...
    public static function fromArray(array $data): EnumSchemaInterface
    {
        ...
        switch ($data['type']) {
            ...
            default:
                throw new \InvalidArgumentException(sprintf("Unknown type value '%s'. Valid values: %s", is_scalar($data['type']) ? $data['type'] : gettype($data['type']), implode(', ', array_keys(self::REGISTRY))));
        }
    }
    /**
