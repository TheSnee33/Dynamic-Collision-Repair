...
namespace Bluehost\Plugin\WP\McpSchema\Common\Traits;
...
trait ValidatesRequiredFields
{
    ...
    {
        $missing = array_filter($requiredFields, static fn(string $field): bool => !array_key_exists($field, $data));
        if (count($missing) > 0) {
            throw new \InvalidArgumentException(sprintf('%s: missing required field(s): %s', static::class, implode(', ', $missing)));
        }
    }
    /**
    ...
    {
        if (!is_int($value)) {
            throw new \InvalidArgumentException(sprintf('Expected int, got %s', gettype($value)));
        }
        return $value;
    }
    /**
    ...
    protected static function asFloat($value): float
    {
        if (!is_float($value) && !is_int($value)) {
            throw new \InvalidArgumentException(sprintf('Expected float, got %s', gettype($value)));
        }
        return (float) $value;
    }
    /**
    ...
    {
        if (!is_bool($value)) {
            throw new \InvalidArgumentException(sprintf('Expected bool, got %s', gettype($value)));
        }
        return $value;
    }
    /**
    ...
    {
        if (!is_array($value)) {
            throw new \InvalidArgumentException(sprintf('Expected array, got %s', gettype($value)));
        }
    ...
    protected static function asObject($value): object
    {
        ...
        if (!is_object($value)) {
            throw new \InvalidArgumentException(sprintf('Expected array or object, got %s', gettype($value)));
        }
        return $value;
    }
    /**
    ...
    protected static function asStringMap($value): array
    {
        ...
        foreach ($value as $key => $v) {
            if (!is_string($v)) {
                throw new \InvalidArgumentException(sprintf('Expected string value for key "%s", got %s', (string) $key, gettype($v)));
            }
        }
    ...
    protected static function asObjectMap($value): array
    {
        ...
        foreach ($value as $key => $v) {
            if (is_array($v)) {
                ...
                throw new \InvalidArgumentException(sprintf('Expected array or object for key "%s", got %s', (string) $key, gettype($v)));
            }
        }
    ...
    {
        if (!is_scalar($value)) {
            throw new \InvalidArgumentException(sprintf('Expected scalar value, got %s', gettype($value)));
        }
        return $value;
    }
    /**
    ...
    protected static function asStringOrNumber($value)
    {
        if (!is_string($value) && !is_int($value) && !is_float($value)) {
            throw new \InvalidArgumentException(sprintf('Expected string or number, got %s', gettype($value)));
        }
        return $value;
    }
    /**
