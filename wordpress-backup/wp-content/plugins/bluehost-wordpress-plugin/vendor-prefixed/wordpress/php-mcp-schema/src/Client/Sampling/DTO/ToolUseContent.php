...
class ToolUseContent extends AbstractDataTransferObject implements SamplingMessageContentBlockInterface
{
     ...
     */
    public function __construct(string $id, string $name, array $input, ?array $_meta = null)
    {
        $this->type = self::TYPE;
        $this->id = $id;
        $this->name = $name;
    ...
    public static function fromArray(array $data): self
    {
        ...
        return new self(self::asString($data['id']), self::asString($data['name']), self::asArray($data['input']), self::asArrayOrNull($data['_meta'] ?? null));
    }
    /**
