...
class ListToolsResult extends PaginatedResult implements ServerResultInterface
{
     ...
     */
    public function __construct(array $tools, ?string $nextCursor = null, ?array $_meta = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $tools = array_map(static fn($item) => is_array($item) ? Tool::fromArray($item) : $item, self::asArray($data['tools']));
        return new self($tools, self::asStringOrNull($data['nextCursor'] ?? null), self::asArrayOrNull($data['_meta'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
