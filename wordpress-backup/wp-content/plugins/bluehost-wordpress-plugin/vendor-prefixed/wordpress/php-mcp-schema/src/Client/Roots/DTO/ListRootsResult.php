...
class ListRootsResult extends Result implements ClientResultInterface
{
     ...
     */
    public function __construct(array $roots, ?array $_meta = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $roots = array_map(static fn($item) => is_array($item) ? Root::fromArray($item) : $item, self::asArray($data['roots']));
        return new self($roots, self::asArrayOrNull($data['_meta'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
