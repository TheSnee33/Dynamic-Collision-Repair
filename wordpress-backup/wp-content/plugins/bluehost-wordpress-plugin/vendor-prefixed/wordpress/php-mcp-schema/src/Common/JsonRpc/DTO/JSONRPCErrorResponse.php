...
class JSONRPCErrorResponse extends AbstractDataTransferObject implements JSONRPCResponseInterface
{
     ...
     */
    public function __construct(string $jsonrpc, Error $error, $id = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $error = is_array($data['error']) ? Error::fromArray(self::asArray($data['error'])) : $data['error'];
        /** @var string|number|null $id */
        $id = isset($data['id']) ? self::asStringOrNumberOrNull($data['id']) : null;
        return new self($jsonrpc, $error, $id);
    }
    /**
