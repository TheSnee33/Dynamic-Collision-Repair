...
namespace Bluehost\Plugin\WP\McpSchema\Client\Sampling\DTO;
...
use Bluehost\Plugin\WP\McpSchema\Common\Tasks\DTO\TaskAugmentedRequestParams;
use Bluehost\Plugin\WP\McpSchema\Common\Traits\ValidatesRequiredFields;
use Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO\Tool;
/**
...
class CreateMessageRequestParams extends TaskAugmentedRequestParams
{
     ...
     */
    public function __construct(array $messages, float $maxTokens, ?TaskMetadata $task = null, ?RequestParamsMeta $_meta = null, ?ModelPreferences $modelPreferences = null, ?string $systemPrompt = null, ?string $includeContext = null, ?float $temperature = null, ?array $stopSequences = null, ?object $metadata = null, ?array $tools = null, ?ToolChoice $toolChoice = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $messages = array_map(static fn($item) => is_array($item) ? SamplingMessage::fromArray($item) : $item, self::asArray($data['messages']));
        /** @var \WP\McpSchema\Client\Tasks\DTO\TaskMetadata|null $task */
        $task = isset($data['task']) ? is_array($data['task']) ? TaskMetadata::fromArray(self::asArray($data['task'])) : $data['task'] : null;
        /** @var \WP\McpSchema\Common\JsonRpc\DTO\RequestParamsMeta|null $_meta */
        $_meta = isset($data['_meta']) ? is_array($data['_meta']) ? RequestParamsMeta::fromArray(self::asArray($data['_meta'])) : $data['_meta'] : null;
        /** @var \WP\McpSchema\Client\Sampling\DTO\ModelPreferences|null $modelPreferences */
        $modelPreferences = isset($data['modelPreferences']) ? is_array($data['modelPreferences']) ? ModelPreferences::fromArray(self::asArray($data['modelPreferences'])) : $data['modelPreferences'] : null;
        /** @var 'none'|'thisServer'|'allServers'|null $includeContext */
        $includeContext = isset($data['includeContext']) ? self::asStringOrNull($data['includeContext']) : null;
        /** @var array<\WP\McpSchema\Server\Tools\DTO\Tool>|null $tools */
        $tools = isset($data['tools']) ? array_map(static fn($item) => is_array($item) ? Tool::fromArray($item) : $item, self::asArray($data['tools'])) : null;
        /** @var \WP\McpSchema\Client\Sampling\DTO\ToolChoice|null $toolChoice */
        $toolChoice = isset($data['toolChoice']) ? is_array($data['toolChoice']) ? ToolChoice::fromArray(self::asArray($data['toolChoice'])) : $data['toolChoice'] : null;
        return new self($messages, self::asFloat($data['maxTokens']), $task, $_meta, $modelPreferences, self::asStringOrNull($data['systemPrompt'] ?? null), $includeContext, self::asFloatOrNull($data['temperature'] ?? null), self::asStringArrayOrNull($data['stopSequences'] ?? null), self::asObjectOrNull($data['metadata'] ?? null), $tools, $toolChoice);
    }
    /**
