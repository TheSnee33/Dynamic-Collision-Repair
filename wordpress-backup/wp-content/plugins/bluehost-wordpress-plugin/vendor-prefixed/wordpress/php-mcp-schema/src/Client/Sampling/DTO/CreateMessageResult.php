...
use Bluehost\Plugin\WP\McpSchema\Client\Lifecycle\Union\ClientResultInterface;
use Bluehost\Plugin\WP\McpSchema\Common\Protocol\DTO\Result;
use Bluehost\Plugin\WP\McpSchema\Common\Protocol\Factory\SamplingMessageContentBlockFactory;
...
class CreateMessageResult extends Result implements ClientResultInterface
{
     ...
     */
    public function __construct(string $model, string $role, array $content, ?array $_meta = null, $stopReason = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $content = array_map(static fn($item) => is_array($item) ? SamplingMessageContentBlockFactory::fromArray($item) : $item, self::asArray($data['content']));
        /** @var "endTurn"|"stopSequence"|"maxTokens"|"toolUse"|string|null $stopReason */
        $stopReason = isset($data['stopReason']) ? $data['stopReason'] : null;
        return new self(self::asString($data['model']), $role, $content, self::asArrayOrNull($data['_meta'] ?? null), $stopReason, self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
