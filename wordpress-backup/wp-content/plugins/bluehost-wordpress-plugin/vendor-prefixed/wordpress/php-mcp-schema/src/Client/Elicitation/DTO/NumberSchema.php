...
class NumberSchema extends AbstractDataTransferObject implements PrimitiveSchemaDefinitionInterface
{
     ...
     */
    public function __construct(string $type, ?string $title = null, ?string $description = null, ?float $minimum = null, ?float $maximum = null, ?float $default = null)
    {
        $this->type = $type;
        $this->title = $title;
    ...
    public static function fromArray(array $data): self
    {
        ...
        return new self($type, self::asStringOrNull($data['title'] ?? null), self::asStringOrNull($data['description'] ?? null), self::asFloatOrNull($data['minimum'] ?? null), self::asFloatOrNull($data['maximum'] ?? null), self::asFloatOrNull($data['default'] ?? null));
    }
    /**
