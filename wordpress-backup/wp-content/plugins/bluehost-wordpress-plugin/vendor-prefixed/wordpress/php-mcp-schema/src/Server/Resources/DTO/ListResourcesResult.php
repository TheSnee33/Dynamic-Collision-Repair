...
class ListResourcesResult extends PaginatedResult implements ServerResultInterface
{
     ...
     */
    public function __construct(array $resources, ?string $nextCursor = null, ?array $_meta = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $resources = array_map(static fn($item) => is_array($item) ? Resource::fromArray($item) : $item, self::asArray($data['resources']));
        return new self($resources, self::asStringOrNull($data['nextCursor'] ?? null), self::asArrayOrNull($data['_meta'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
