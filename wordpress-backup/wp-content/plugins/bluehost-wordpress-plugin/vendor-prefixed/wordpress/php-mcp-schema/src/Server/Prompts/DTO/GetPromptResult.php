...
class GetPromptResult extends Result implements ServerResultInterface
{
     ...
     */
    public function __construct(array $messages, ?array $_meta = null, ?string $description = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $messages = array_map(static fn($item) => is_array($item) ? PromptMessage::fromArray($item) : $item, self::asArray($data['messages']));
        return new self($messages, self::asArrayOrNull($data['_meta'] ?? null), self::asStringOrNull($data['description'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
