...
use Bluehost\Plugin\WP\McpSchema\Client\Lifecycle\DTO\ClientCapabilities;
...
class InitializeRequestParams extends RequestParams
{
     ...
     */
    public function __construct(string $protocolVersion, ClientCapabilities $capabilities, Implementation $clientInfo, ?RequestParamsMeta $_meta = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $capabilities = is_array($data['capabilities']) ? ClientCapabilities::fromArray(self::asArray($data['capabilities'])) : $data['capabilities'];
        /** @var \WP\McpSchema\Common\Lifecycle\DTO\Implementation $clientInfo */
        $clientInfo = is_array($data['clientInfo']) ? Implementation::fromArray(self::asArray($data['clientInfo'])) : $data['clientInfo'];
        ...
        return new self(self::asString($data['protocolVersion']), $capabilities, $clientInfo, $_meta);
    }
    /**
