...
use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\DTO\ElicitRequestFormParams;
use Bluehost\Plugin\WP\McpSchema\Client\Elicitation\DTO\ElicitRequestURLParams;
/**
...
final class ElicitRequestParamsFactory
{
     ...
     */
    public const REGISTRY = ['form' => ElicitRequestFormParams::class, 'url' => ElicitRequestURLParams::class];
    /**
    ...
    public static function fromArray(array $data): ElicitRequestParamsInterface
    {
        ...
        if (!isset(self::REGISTRY[$mode])) {
            throw new \InvalidArgumentException(sprintf("Unknown mode value '%s'. Valid values: %s", $mode, implode(', ', array_keys(self::REGISTRY))));
        }
