...
class Task extends AbstractDataTransferObject
{
     ...
     */
    public function __construct(string $taskId, string $status, string $createdAt, string $lastUpdatedAt, ?int $ttl, ?string $statusMessage = null, ?int $pollInterval = null)
    {
        $this->taskId = $taskId;
        $this->status = $status;
    ...
    public static function fromArray(array $data): self
    {
        ...
        return new self(self::asString($data['taskId']), $status, self::asString($data['createdAt']), self::asString($data['lastUpdatedAt']), self::asInt($data['ttl']), self::asStringOrNull($data['statusMessage'] ?? null), self::asIntOrNull($data['pollInterval'] ?? null));
    }
    /**
