...
namespace Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO;

use Bluehost\Plugin\WP\McpSchema\Common\Core\DTO\Icon;
use Bluehost\Plugin\WP\McpSchema\Common\Protocol\DTO\BaseMetadata;
...
class Tool extends BaseMetadata
{
     ...
     */
    public function __construct(string $name, ToolInputSchema $inputSchema, ?string $title = null, ?string $description = null, ?ToolExecution $execution = null, ?ToolOutputSchema $outputSchema = null, ?ToolAnnotations $annotations = null, ?array $_meta = null, ?array $icons = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $inputSchema = is_array($data['inputSchema']) ? ToolInputSchema::fromArray(self::asArray($data['inputSchema'])) : $data['inputSchema'];
        /** @var \WP\McpSchema\Server\Tools\DTO\ToolExecution|null $execution */
        $execution = isset($data['execution']) ? is_array($data['execution']) ? ToolExecution::fromArray(self::asArray($data['execution'])) : $data['execution'] : null;
        /** @var \WP\McpSchema\Server\Tools\DTO\ToolOutputSchema|null $outputSchema */
        $outputSchema = isset($data['outputSchema']) ? is_array($data['outputSchema']) ? ToolOutputSchema::fromArray(self::asArray($data['outputSchema'])) : $data['outputSchema'] : null;
        /** @var \WP\McpSchema\Server\Tools\DTO\ToolAnnotations|null $annotations */
        $annotations = isset($data['annotations']) ? is_array($data['annotations']) ? ToolAnnotations::fromArray(self::asArray($data['annotations'])) : $data['annotations'] : null;
        /** @var array<\WP\McpSchema\Common\Core\DTO\Icon>|null $icons */
        $icons = isset($data['icons']) ? array_map(static fn($item) => is_array($item) ? Icon::fromArray($item) : $item, self::asArray($data['icons'])) : null;
        return new self(self::asString($data['name']), $inputSchema, self::asStringOrNull($data['title'] ?? null), self::asStringOrNull($data['description'] ?? null), $execution, $outputSchema, $annotations, self::asArrayOrNull($data['_meta'] ?? null), $icons);
    }
    /**
