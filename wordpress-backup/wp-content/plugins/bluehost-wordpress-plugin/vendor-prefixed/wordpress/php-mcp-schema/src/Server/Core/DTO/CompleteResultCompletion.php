...
class CompleteResultCompletion extends AbstractDataTransferObject
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        if (is_array($data['values']) && count($data['values']) > self::MAX_VALUES) {
            throw new \InvalidArgumentException(sprintf('%s::values must not exceed %d items, got %d', static::class, self::MAX_VALUES, count($data['values'])));
        }
        return new self(self::asStringArray($data['values']), self::asIntOrNull($data['total'] ?? null), self::asBoolOrNull($data['hasMore'] ?? null));
    }
    /**
