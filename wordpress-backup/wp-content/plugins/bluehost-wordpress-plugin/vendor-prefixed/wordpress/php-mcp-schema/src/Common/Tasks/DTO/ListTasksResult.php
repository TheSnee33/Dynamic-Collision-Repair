...
use Bluehost\Plugin\WP\McpSchema\Client\Tasks\DTO\Task;
...
class ListTasksResult extends PaginatedResult implements ClientResultInterface, ServerResultInterface
{
     ...
     */
    public function __construct(array $tasks, ?string $nextCursor = null, ?array $_meta = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $tasks = array_map(static fn($item) => is_array($item) ? Task::fromArray($item) : $item, self::asArray($data['tasks']));
        return new self($tasks, self::asStringOrNull($data['nextCursor'] ?? null), self::asArrayOrNull($data['_meta'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
