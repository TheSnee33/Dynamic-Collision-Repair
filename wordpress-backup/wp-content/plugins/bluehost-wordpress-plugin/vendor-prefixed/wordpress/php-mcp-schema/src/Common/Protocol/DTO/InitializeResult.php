...
namespace Bluehost\Plugin\WP\McpSchema\Common\Protocol\DTO;

use Bluehost\Plugin\WP\McpSchema\Common\Lifecycle\DTO\Implementation;
use Bluehost\Plugin\WP\McpSchema\Common\Traits\ValidatesRequiredFields;
use Bluehost\Plugin\WP\McpSchema\Server\Lifecycle\DTO\ServerCapabilities;
...
class InitializeResult extends Result implements ServerResultInterface
{
     ...
     */
    public function __construct(string $protocolVersion, ServerCapabilities $capabilities, Implementation $serverInfo, ?array $_meta = null, ?string $instructions = null, ?array $additionalProperties = null)
    {
    ...
    public static function fromArray(array $data): self
    {
        ...
        $capabilities = is_array($data['capabilities']) ? ServerCapabilities::fromArray(self::asArray($data['capabilities'])) : $data['capabilities'];
        /** @var \WP\McpSchema\Common\Lifecycle\DTO\Implementation $serverInfo */
        $serverInfo = is_array($data['serverInfo']) ? Implementation::fromArray(self::asArray($data['serverInfo'])) : $data['serverInfo'];
        return new self(self::asString($data['protocolVersion']), $capabilities, $serverInfo, self::asArrayOrNull($data['_meta'] ?? null), self::asStringOrNull($data['instructions'] ?? null), self::additionalFields($data, self::KNOWN_KEYS));
    }
    /**
