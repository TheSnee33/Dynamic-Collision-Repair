...
class ModelPreferences extends AbstractDataTransferObject
{
    ...
    public static function fromArray(array $data): self
    {
        ...
        $hints = isset($data['hints']) ? array_map(static fn($item) => is_array($item) ? ModelHint::fromArray($item) : $item, self::asArray($data['hints'])) : null;
        return new self($hints, self::asFloatOrNull($data['costPriority'] ?? null), self::asFloatOrNull($data['speedPriority'] ?? null), self::asFloatOrNull($data['intelligencePriority'] ?? null));
    }
    /**
