...
use Bluehost\Plugin\WP\MCP\Infrastructure\Observability\McpObservabilityHelperTrait;
use Bluehost\Plugin\WP\McpSchema\Common\AbstractDataTransferObject;
use Bluehost\Plugin\WP\McpSchema\Common\Content\DTO\TextContent;
use Bluehost\Plugin\WP\McpSchema\Common\JsonRpc\DTO\JSONRPCErrorResponse;
use Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO\CallToolResult;
/**
 * Service for routing MCP requests to appropriate handlers.
 *
...
class RequestRouter
{
    ...
    public function route_request(string $method, array $params, $request_id = 0, string $transport_name = 'unknown', ?HttpRequestContext $http_context = null): array
    {
        ...
        $common_tags = array('method' => $method, 'transport' => $transport_name, 'server_id' => $this->context->mcp_server->get_server_id(), 'params' => $this->sanitize_params_for_logging($params), 'request_id' => $request_id, 'session_id' => $http_context ? $http_context->session_id : null);
        $handlers = array('initialize' => function () use ($params, $request_id, $http_context, &$new_session_id) {
            return $this->handle_initialize_with_session($params, $request_id, $http_context, $new_session_id);
        }, 'ping' => fn() => $this->context->system_handler->ping(), 'tools/list' => fn() => $this->context->tools_handler->list_tools(), 'tools/list/all' => fn() => $this->context->tools_handler->list_all_tools(), 'tools/call' => fn() => $this->context->tools_handler->call_tool($params, $request_id), 'resources/list' => fn() => $this->context->resources_handler->list_resources(), 'resources/read' => fn() => $this->context->resources_handler->read_resource($params, $request_id), 'prompts/list' => fn() => $this->context->prompts_handler->list_prompts(), 'prompts/get' => fn() => $this->context->prompts_handler->get_prompt($params, $request_id));
        try {
            $handler_result = isset($handlers[$method]) ? $handlers[$method]() : $this->create_method_not_found_error($method, $request_id);
            // Calculate request duration.
            $duration = (microtime(\true) - $start_time) * 1000;
            ...
            if ($handler_result instanceof AbstractDataTransferObject) {
                ...
                $status = 'success';
                if ($handler_result instanceof CallToolResult && \true === $handler_result->getIsError()) {
                    $status = 'error';
            ...
            $this->context->error_handler->log(sprintf('Handler for method "%s" returned unexpected type: %s', $method, $actual_type), array('method' => $method, 'actual_type' => $actual_type));
            ...
            $tags = array_merge($common_tags, $component_tags, array('status' => 'error', 'error_type' => get_class($exception), 'error_category' => $this->categorize_error($exception)));
            $this->context->observability_handler->record_event('mcp.request', $tags, $duration);
    ...
    private function resolve_component_observability_context(string $method, array $params): array
    {
        ...
        switch ($method) {
            case 'tools/call':
                ...
                return array('component_type' => 'tool', 'tool_name' => $tool_name);
            case 'prompts/get':
                ...
                return array('component_type' => 'prompt', 'prompt_name' => $prompt_name);
            case 'resources/read':
                ...
                return array('component_type' => 'resource', 'resource_uri' => $resource_uri);
        }
        return array();
    }
    /**
    ...
    private function handle_initialize_with_session(array $params, $request_id, ?HttpRequestContext $http_context, ?string &$new_session_id = null): AbstractDataTransferObject
    {
        ...
        if ($http_context && !$http_context->session_id) {
            ...
            if (is_array($session_result)) {
                ...
                return McpErrorFactory::create_error_response($request_id, isset($error['code']) ? (int) $error['code'] : McpErrorFactory::INTERNAL_ERROR, (string) ($error['message'] ?? __('Failed to create session', 'mcp-adapter')), $error['data'] ?? null);
            }
    ...
    {
        $error_categories = array(\ArgumentCountError::class => 'arguments', \TypeError::class => 'type', \InvalidArgumentException::class => 'validation', \LogicException::class => 'logic', \RuntimeException::class => 'execution', \Error::class => 'system');
