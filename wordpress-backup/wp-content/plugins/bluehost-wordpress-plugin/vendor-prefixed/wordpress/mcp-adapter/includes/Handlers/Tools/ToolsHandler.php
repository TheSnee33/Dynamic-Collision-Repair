...
namespace Bluehost\Plugin\WP\MCP\Handlers\Tools;

use Bluehost\Plugin\WP\MCP\Core\McpServer;
use Bluehost\Plugin\WP\MCP\Domain\Utils\ContentBlockHelper;
...
use Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO\ListToolsResult;
/**
...
class ToolsHandler
{
    ...
    public function call_tool(array $params, $request_id = 0)
    {
        ...
        try {
            ...
            if (!$mcp_tool) {
                $this->mcp->get_error_handler()->log('Tool not found', array('tool_name' => $tool_name), 'warning');
            ...
            if (\true !== $permission) {
                ...
                if (is_wp_error($permission)) {
                    ...
                    $this->mcp->get_error_handler()->log('Tool permission check failed', array('tool_name' => $tool_name, 'error_code' => $permission->get_error_code(), 'error_message' => $permission->get_error_message(), 'error_data' => $permission->get_error_data(), 'failure_reason' => FailureReason::PERMISSION_CHECK_FAILED));
                }
            ...
            if (is_array($result) && array_key_exists('success', $result) && \false === $result['success'] && isset($result['error']) && is_string($result['error']) && '' !== trim($result['error'])) {
            ...
            if (isset($result['type']) && 'resource' === $result['type']) {
                ...
                if (is_string($uri) && '' !== $uri) {
                    if (isset($resource_item['text']) && is_string($resource_item['text'])) {
                        return CallToolResult::fromArray(array('content' => array(ContentBlockHelper::embedded_text_resource($uri, $resource_item['text'], is_string($mime_type) ? $mime_type : null)), 'isError' => \false));
                    }
                    if (isset($resource_item['blob']) && is_string($resource_item['blob'])) {
                        return CallToolResult::fromArray(array('content' => array(ContentBlockHelper::embedded_blob_resource($uri, $resource_item['blob'], is_string($mime_type) ? $mime_type : null)), 'isError' => \false));
                    }
                }
            }
            ...
            if (isset($result['type']) && 'image' === $result['type'] && isset($result['results'])) {
                ...
                return CallToolResult::fromArray(array('content' => array(ContentBlockHelper::image($image_data, $mime_type)), 'structuredContent' => null, 'isError' => \false));
            }
            ...
            return CallToolResult::fromArray(array('content' => array(ContentBlockHelper::text($json_text)), 'structuredContent' => $result, 'isError' => \false));
        } catch (\Throwable $exception) {
            $this->mcp->get_error_handler()->log('Error calling tool', array('tool' => $request_params['name'], 'exception' => $exception->getMessage()));
    ...
    {
        return CallToolResult::fromArray(array('content' => array(ContentBlockHelper::text($message)), 'structuredContent' => null, 'isError' => \true));
    }
}
