...
use Bluehost\Plugin\WP\MCP\Domain\Prompts\Contracts\McpPromptBuilderInterface;
...
use Bluehost\Plugin\WP\MCP\Infrastructure\Observability\FailureReason;
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\Prompt as PromptDto;
use Bluehost\Plugin\WP\McpSchema\Server\Resources\DTO\Resource as ResourceDto;
use Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO\Tool as ToolDto;
use WP_Error;
/**
...
class McpComponentRegistry
{
     ...
     */
    public function __construct(McpServer $mcp_server, McpErrorHandlerInterface $error_handler, McpObservabilityHandlerInterface $observability_handler)
    {
    ...
    private function register_single_tool($tool_item): void
    {
        ...
        $this->error_handler->log(sprintf('Invalid tool registration item: expected McpTool instance or string ability name, got %s.', is_object($tool_item) ? get_class($tool_item) : gettype($tool_item)), array('McpComponentRegistry::register_single_tool'), 'warning');
    }
    /**
    ...
    private function add_mcp_tool(McpTool $mcp_tool): void
    {
        ...
        if (isset($this->mcp_tools[$tool_name])) {
            $this->error_handler->log("Tool with name '{$tool_name}' already registered, skipping duplicate.", array('McpComponentRegistry::add_mcp_tool'), 'warning');
            return;
        }
    ...
    private function track_registration(string $type, string $name, string $status, array $extra = array()): void
    {
        ...
        $event_data = array_merge(array('status' => $status, 'component_type' => $type, 'component_name' => $name, 'server_id' => $this->mcp_server->get_server_id()), $extra);
    ...
    private function register_ability_tool(string $ability_name): void
    {
        ...
        if (is_wp_error($mcp_tool)) {
            ...
            $this->track_registration('ability_tool', $ability_name, 'failed', array('error_code' => $mcp_tool->get_error_code()));
            return;
        }
    ...
    private function register_single_resource($resource_item): void
    {
        ...
        // Case 2: Invalid type.
        $this->error_handler->log(sprintf('Invalid resource registration item: expected McpResource instance or string ability name, got %s.', is_object($resource_item) ? get_class($resource_item) : gettype($resource_item)), array('McpComponentRegistry::register_single_resource'), 'warning');
    }
    /**
    ...
    private function add_mcp_resource(McpResource $mcp_resource): bool
    {
        ...
        if (isset($this->mcp_resources[$uri])) {
            $this->error_handler->log("Resource with URI '{$uri}' already registered, skipping duplicate.", array('McpComponentRegistry::add_mcp_resource'), 'warning');
            return \false;
        }
    ...
    private function register_ability_resource(string $ability_name): void
    {
        ...
        if (is_wp_error($mcp_resource)) {
            ...
            $this->track_registration('resource', $ability_name, 'failed', array('error_code' => $mcp_resource->get_error_code()));
            return;
        }
        ...
        if ($added) {
            ...
            /** @var \WP\McpSchema\Server\Resources\DTO\Resource $resource_dto */
            $resource_dto = $mcp_resource->get_protocol_dto();
            $this->track_registration('resource', $ability_name, 'failed', array('failure_reason' => FailureReason::DUPLICATE_URI, 'duplicate_uri' => $resource_dto->getUri()));
        }
    }
    /**
    ...
    private function register_single_prompt($prompt_item): void
    {
        ...
        if (is_string($prompt_item)) {
            ...
            if (class_exists($prompt_item) && in_array(McpPromptBuilderInterface::class, class_implements($prompt_item) ?: array(), \true)) {
        ...
        // Case 3: Invalid type.
        $this->error_handler->log(sprintf('Invalid prompt registration item: expected McpPrompt, McpPromptBuilderInterface, or string, got %s.', is_object($prompt_item) ? get_class($prompt_item) : gettype($prompt_item)), array('McpComponentRegistry::register_single_prompt'), 'warning');
    }
    /**
    ...
    private function add_mcp_prompt(McpPrompt $mcp_prompt): void
    {
        ...
        if (isset($this->mcp_prompts[$prompt_name])) {
            $this->error_handler->log("Prompt with name '{$prompt_name}' already registered, skipping duplicate.", array('McpComponentRegistry::add_mcp_prompt'), 'warning');
            return;
        }
    ...
    private function register_builder_instance(McpPromptBuilderInterface $builder): void
    {
        ...
        if ($mcp_prompt instanceof WP_Error) {
            ...
            $this->track_registration('prompt', $prompt_name, 'failed', array('error_code' => $mcp_prompt->get_error_code()));
            return;
        }
    ...
    private function register_ability_prompt(string $ability_name): void
    {
        ...
        if (is_wp_error($mcp_prompt)) {
            ...
            $this->track_registration('prompt', $ability_name, 'failed', array('error_code' => $mcp_prompt->get_error_code()));
            return;
        }
        $this->add_mcp_prompt($mcp_prompt);
    ...
    {
        return array_map(static fn(McpTool $mcp_tool): ToolDto => $mcp_tool->get_protocol_dto(), $this->mcp_tools);
    }
    /**
    ...
    {
        return array_map(static fn(McpResource $mcp_resource): ResourceDto => $mcp_resource->get_protocol_dto(), $this->mcp_resources);
    }
    /**
    ...
    {
        return array_map(static fn(McpPrompt $mcp_prompt): PromptDto => $mcp_prompt->get_protocol_dto(), $this->mcp_prompts);
    }
    /**
