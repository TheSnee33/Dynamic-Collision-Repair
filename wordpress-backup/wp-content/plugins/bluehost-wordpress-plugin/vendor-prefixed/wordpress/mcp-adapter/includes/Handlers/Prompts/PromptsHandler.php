...
namespace Bluehost\Plugin\WP\MCP\Handlers\Prompts;

use Bluehost\Plugin\WP\MCP\Core\McpServer;
use Bluehost\Plugin\WP\MCP\Handlers\HandlerHelperTrait;
use Bluehost\Plugin\WP\MCP\Infrastructure\ErrorHandling\McpErrorFactory;
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\GetPromptResult;
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\ListPromptsResult;
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\Prompt as PromptDto;
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\PromptMessage;
/**
...
class PromptsHandler
{
    ...
    public function get_prompt(array $params, $request_id = 0)
    {
        ...
        try {
            ...
            $this->mcp->get_error_handler()->log('Prompt execution failed', array('prompt_name' => $prompt_name, 'arguments' => $arguments, 'error' => $e->getMessage()));
     ...
     */
    private function normalize_result_to_dto(array $result, PromptDto $prompt, string $prompt_name): GetPromptResult
    {
        // Tier 1: Full MCP format with 'messages' array.
     ...
     * @return \WP\McpSchema\Server\Prompts\DTO\GetPromptResult
     */
    private function normalize_tier1_messages(array $result, PromptDto $prompt, string $prompt_name): GetPromptResult
    {
        $message_dtos = array();
        foreach ($result['messages'] as $index => $message) {
            if (!is_array($message)) {
                $this->mcp->get_error_handler()->log('Invalid message structure in prompt result, skipping', array('prompt_name' => $prompt_name, 'message_index' => $index, 'message_type' => gettype($message)), 'warning');
                continue;
            }
        ...
        if (empty($message_dtos)) {
            $message_dtos[] = PromptMessage::fromArray(array('role' => self::$default_role, 'content' => array('type' => 'text', 'text' => '(No messages returned)')));
        }
        return GetPromptResult::fromArray(array('messages' => $message_dtos, 'description' => $result['description'] ?? $prompt->getDescription()));
    }
    /**
     * Tier 2: Simple 'text' shorthand.
     *
    ...
    {
        $content = array('type' => 'text', 'text' => (string) $result['text']);
        ...
        $message_dto = PromptMessage::fromArray(array('role' => self::$default_role, 'content' => $content));
        return GetPromptResult::fromArray(array('messages' => array($message_dto), 'description' => $result['description'] ?? $prompt->getDescription()));
    }
    /**
     * Tier 3: Single message with 'role' and 'content'.
     *
     ...
     * @return \WP\McpSchema\Server\Prompts\DTO\GetPromptResult
     */
    private function normalize_tier3_single_message(array $result, PromptDto $prompt, string $prompt_name): GetPromptResult
    {
    ...
    private function normalize_tier4_texts(array $result, PromptDto $prompt): GetPromptResult
    {
        ...
        foreach ($result['texts'] as $text) {
            ...
            $message_dtos[] = PromptMessage::fromArray(array('role' => $role, 'content' => array('type' => 'text', 'text' => $text)));
        }
        // Ensure we have at least one message.
        if (empty($message_dtos)) {
            $message_dtos[] = PromptMessage::fromArray(array('role' => $role, 'content' => array('type' => 'text', 'text' => '(No texts provided)')));
        }
        return GetPromptResult::fromArray(array('messages' => $message_dtos, 'description' => $result['description'] ?? $prompt->getDescription()));
    }
    /**
     ...
     * @return \WP\McpSchema\Server\Prompts\DTO\GetPromptResult
     */
    private function normalize_tier5_fallback(array $result, PromptDto $prompt, string $prompt_name): GetPromptResult
    {
        // Log observability event for fallback normalization.
        $this->mcp->get_observability_handler()->record_event('prompt_result_fallback_normalization', array('prompt_name' => $prompt_name, 'result_keys' => array_keys($result)));
        $json_content = wp_json_encode($result, \JSON_PRETTY_PRINT);
        if (\false === $json_content) {
            $json_content = '{}';
        }
        $message_dto = PromptMessage::fromArray(array('role' => self::$default_role, 'content' => array('type' => 'text', 'text' => $json_content)));
        return GetPromptResult::fromArray(array('messages' => array($message_dto), 'description' => $prompt->getDescription()));
    }
    // =========================================================================
    // Validation Helpers
    // =========================================================================
    ...
    private function validate_and_create_message(array $message, string $prompt_name): PromptMessage
    {
        ...
        if (!is_array($content)) {
            ...
            $content = array('type' => 'text', 'text' => is_string($content) ? $content : (string) $content);
        }
        $content = $this->validate_content_type($content, $prompt_name);
        return PromptMessage::fromArray(array('role' => $role, 'content' => $content));
    }
    /**
    ...
    private function validate_content_type(array $content, string $prompt_name): array
    {
        ...
        if (null === $type || '' === $type) {
            $this->mcp->get_error_handler()->log('Missing content type in prompt result, defaulting to text', array('prompt_name' => $prompt_name), 'warning');
            $text = isset($content['text']) ? (string) $content['text'] : wp_json_encode($content, \JSON_PRETTY_PRINT);
            return array('type' => 'text', 'text' => \false === $text ? '{}' : $text);
        }
        // Check if type is valid.
        if (!in_array($type, self::$valid_content_types, \true)) {
            $this->mcp->get_error_handler()->log('Invalid content type in prompt result, converting to text', array('prompt_name' => $prompt_name, 'invalid_type' => $type, 'valid_types' => self::$valid_content_types), 'warning');
    ...
    {
        if (in_array($role, self::$valid_roles, \true)) {
            return $role;
        }
        if ('' !== $prompt_name) {
            $this->mcp->get_error_handler()->log('Invalid role in prompt message, defaulting to user', array('prompt_name' => $prompt_name, 'invalid_role' => $role, 'valid_roles' => self::$valid_roles), 'warning');
        }
