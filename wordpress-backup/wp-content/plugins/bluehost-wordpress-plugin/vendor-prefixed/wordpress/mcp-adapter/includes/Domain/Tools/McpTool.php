...
use Bluehost\Plugin\WP\McpSchema\Server\Tools\DTO\ToolAnnotations;
use WP_Error;
/**
...
final class McpTool implements McpComponentInterface
{
    ...
    public static function fromArray(array $config)
    {
        ...
        $tool_data = array('name' => $config['name'], 'inputSchema' => $input_schema);
        // Optional fields.
        ...
        try {
            ...
            return new WP_Error('mcp_tool_dto_creation_failed', sprintf(
        ...
        $instance->observability_context = array('component_type' => 'tool', 'tool_name' => $config['name'], 'source' => 'array');
        return $instance;
    }
    /**
    ...
    public static function fromAbility(\WP_Ability $ability)
    {
        ...
        $instance->observability_context = array('component_type' => 'tool', 'tool_name' => $tool_data['tool']->getName(), 'ability_name' => $ability->get_name(), 'source' => 'ability');
        return $instance;
    }
    // =========================================================================
    ...
    {
        $is_transformed = \true === ($this->adapter_meta['output_schema_transformed'] ?? \false);
        if (!$is_transformed) {
            return $result;
        }
    ...
    public function check_permission($arguments)
    {
        ...
        return new WP_Error('mcp_permission_denied', 'Access denied.', array('failure_reason' => FailureReason::NO_PERMISSION_STRATEGY, 'tool_name' => $this->tool->getName()));
    }
    // =========================================================================
