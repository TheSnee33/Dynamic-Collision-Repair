...
use Bluehost\Plugin\WP\MCP\Core\McpVersionNegotiator;
...
class HttpRequestHandler
{
    ...
    public function handle_request(HttpRequestContext $context): \WP_REST_Response
    {
        ...
        // Method not allowed
        return new \WP_REST_Response(McpErrorFactory::invalid_request(null, 'Method not allowed')->toArray(), 405);
    }
    /**
    ...
    private function handle_mcp_request(HttpRequestContext $context): \WP_REST_Response
    {
        try {
            ...
            $this->transport_context->mcp_server->get_error_handler()->log('Unexpected error in handle_mcp_request', array('transport' => static::class, 'server_id' => $this->transport_context->mcp_server->get_server_id(), 'error' => $exception->getMessage()));
    ...
    private function process_mcp_messages(HttpRequestContext $context): \WP_REST_Response
    {
        ...
        $response_body = JsonRpcResponseBuilder::process_messages($messages, $is_batch_request, function (array $message) use ($context) {
    ...
    private function process_jsonrpc_request(array $message, HttpRequestContext $context): array
    {
        ...
        // Preserve original scalar ID (string, number, or null)
        ...
        $result = $this->transport_context->request_router->route_request($method, $params, $request_id, $this->get_transport_name(), $context);
        ...
        if (isset($result['_session_id'])) {
            ...
            // Remove from actual response data
        }
    ...
    private function validate_protocol_version_header(HttpRequestContext $context): ?array
    {
        ...
        return McpErrorFactory::create_error(McpErrorFactory::INVALID_REQUEST, sprintf('Bad Request: Unsupported protocol version: %s (supported versions: %s)', $context->protocol_version, implode(', ', McpVersionNegotiator::SUPPORTED_PROTOCOL_VERSIONS)))->toArray();
    }
    /**
    ...
    private function add_session_header_to_response(string $session_id): void
    {
        ...
        add_filter('rest_post_dispatch', static function ($response) use ($session_id) {
