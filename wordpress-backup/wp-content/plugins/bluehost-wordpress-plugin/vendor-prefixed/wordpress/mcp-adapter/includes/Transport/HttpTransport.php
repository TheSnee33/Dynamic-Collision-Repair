...
class HttpTransport implements McpRestTransportInterface
{
    ...
    public function register_routes(): void
    {
        ...
        register_rest_route($server->get_server_route_namespace(), $server->get_server_route(), array('methods' => array('POST', 'GET', 'DELETE'), 'callback' => array($this, 'handle_request'), 'permission_callback' => array($this, 'check_permission')));
    }
    /**
    ...
    public function check_permission(\WP_REST_Request $request)
    {
        ...
        if (null !== $transport_context->transport_permission_callback) {
            try {
                ...
                $this->request_handler->get_transport_context()->error_handler->log('Permission callback returned WP_Error: ' . $result->get_error_message(), array('HttpTransport::check_permission'));
                return \false;
            } catch (\Throwable $e) {
                // Log the error and deny access (fail-closed)
        ...
        $user_has_capability = current_user_can($user_capability);
        // phpcs:ignore WordPress.WP.Capabilities.Undetermined -- Capability is filtered and defaults to 'read'
        if (!$user_has_capability) {
            $user_id = get_current_user_id();
            $this->request_handler->get_transport_context()->error_handler->log(sprintf('Permission denied for MCP API access. User ID %d does not have capability "%s"', $user_id, $user_capability), array('HttpTransport::check_permission'));
        }
