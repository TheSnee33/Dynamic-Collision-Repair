...
class McpValidator
{
     ...
     * @var array<string>
     */
    private static array $allowed_icon_mime_types = array('image/png', 'image/jpeg', 'image/jpg', 'image/svg+xml', 'image/webp');
    /**
    ...
    public static function validate_base64(string $content): bool
    {
        ...
        return base64_decode($content, \true) !== \false;
     ...
     *
     */
    public static function validate_icons_array(array $icons, bool $log_warnings = \true): array
    {
        $valid_icons = array();
        $all_errors = array();
        foreach ($icons as $index => $icon) {
            if (!is_array($icon)) {
                $all_errors[] = array('index' => $index, 'errors' => array(__('Icon must be an array', 'mcp-adapter')));
                continue;
            }
            ...
            if (empty($errors)) {
                ...
                $all_errors[] = array('index' => $index, 'errors' => $errors);
                if ($log_warnings) {
                    // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
                    error_log(sprintf('MCP Adapter: Invalid icon at index %d skipped: %s', $index, implode('; ', $errors)));
                }
            }
        }
        return array('valid' => $valid_icons, 'errors' => $all_errors);
    }
    /**
    ...
    {
        return in_array(strtolower(trim($mime_type)), self::$allowed_icon_mime_types, \true);
    }
    /**
    ...
    {
        return in_array(strtolower(trim($theme)), array('light', 'dark'), \true);
    }
    /**
    ...
    {
        return in_array($role, array('user', 'assistant'), \true);
    }
    /**
