...
class DefaultServerFactory
{
    ...
    public static function create(): void
    {
        ...
        $result = $adapter->create_server($config['server_id'], $config['server_route_namespace'], $config['server_route'], $config['server_name'], $config['server_description'], $config['server_version'], $config['mcp_transports'], $config['error_handler'], $config['observability_handler'], $config['tools'], $config['resources'], $config['prompts']);
        ...
        _doing_it_wrong(__METHOD__, sprintf('MCP Adapter: Failed to create default server. Error: %s (Code: %s)', esc_html($result->get_error_message()), esc_html((string) $result->get_error_code())), '0.5.0');
    }
    /**
