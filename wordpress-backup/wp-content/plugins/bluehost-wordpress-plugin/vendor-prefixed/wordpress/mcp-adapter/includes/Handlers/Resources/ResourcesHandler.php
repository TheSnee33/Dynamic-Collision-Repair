...
namespace Bluehost\Plugin\WP\MCP\Handlers\Resources;
...
use Bluehost\Plugin\WP\McpSchema\Common\Protocol\DTO\BlobResourceContents;
use Bluehost\Plugin\WP\McpSchema\Common\Protocol\DTO\TextResourceContents;
use Bluehost\Plugin\WP\McpSchema\Server\Resources\DTO\ListResourcesResult;
use Bluehost\Plugin\WP\McpSchema\Server\Resources\DTO\ReadResourceResult;
/**
...
class ResourcesHandler
{
    ...
    public function read_resource(array $params, $request_id = 0)
    {
        ...
        try {
            ...
            return ReadResourceResult::fromArray(array('contents' => $content_dtos));
        } catch (\Throwable $exception) {
            $this->mcp->get_error_handler()->log('Error reading resource', array('uri' => $uri, 'exception' => $exception->getMessage()));
    ...
    private function convert_contents_to_dtos($contents, string $uri): array
    {
        ...
        return array(TextResourceContents::fromArray(array('uri' => $uri, 'text' => $text)));
    }
    /**
    ...
    private function create_content_dto(array $item, string $default_uri)
    {
        ...
        if (isset($item['blob'])) {
            return BlobResourceContents::fromArray(array('uri' => $item_uri, 'blob' => (string) $item['blob'], 'mimeType' => is_string($mime_type) ? $mime_type : null));
        }
        // Default to TextResourceContents.
        $text = $item['text'] ?? '';
        return TextResourceContents::fromArray(array('uri' => $item_uri, 'text' => (string) $text, 'mimeType' => is_string($mime_type) ? $mime_type : null));
    }
}
