...
namespace Bluehost\Plugin\WP\MCP\Domain\Utils;
...
class McpAnnotationMapper
{
    ...
    private static array $mcp_annotations = array(
        ...
        'audience' => array('type' => 'array', 'features' => array('resource'), 'ability_property' => null),
        'lastModified' => array('type' => 'string', 'features' => array('resource'), 'ability_property' => null),
        'priority' => array('type' => 'number', 'features' => array('resource'), 'ability_property' => null),
        // Tool-specific annotations (ToolAnnotations type per MCP 2025-11-25 spec).
        'readOnlyHint' => array('type' => 'boolean', 'features' => array('tool'), 'ability_property' => 'readonly'),
        'destructiveHint' => array('type' => 'boolean', 'features' => array('tool'), 'ability_property' => 'destructive'),
        'idempotentHint' => array('type' => 'boolean', 'features' => array('tool'), 'ability_property' => 'idempotent'),
        'openWorldHint' => array('type' => 'boolean', 'features' => array('tool'), 'ability_property' => null),
        'title' => array('type' => 'string', 'features' => array('tool'), 'ability_property' => null),
    );
    /**
    ...
    public static function map(array $ability_annotations, string $feature_type): array
    {
        ...
        foreach (self::$mcp_annotations as $mcp_field => $config) {
            if (!in_array($feature_type, $config['features'], \true)) {
                continue;
            }
            $value = self::resolve_annotation_value($ability_annotations, $mcp_field, $config['ability_property']);
            if (null === $value) {
                continue;
            }
