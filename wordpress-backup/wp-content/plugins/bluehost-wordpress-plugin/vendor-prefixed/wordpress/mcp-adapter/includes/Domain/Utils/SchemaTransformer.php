...
class SchemaTransformer
{
    ...
    public static function transform_to_object_schema(?array $schema, string $wrapper_key = 'input'): array
    {
        ...
        if (empty($schema)) {
            return array('schema' => array('type' => 'object'), 'was_transformed' => \false, 'wrapper_property' => null);
        }
        // If no type is specified, add 'object' type since MCP requires it.
        if (!isset($schema['type'])) {
            $schema['type'] = 'object';
            return array('schema' => $schema, 'was_transformed' => \false, 'wrapper_property' => null);
        }
        ...
        return array('schema' => self::wrap_in_object($schema, $wrapper_key), 'was_transformed' => \true, 'wrapper_property' => $wrapper_key);
    }
    /**
    ...
    {
        return array('type' => 'object', 'properties' => array($wrapper_key => $schema), 'required' => array($wrapper_key));
    }
    /**
