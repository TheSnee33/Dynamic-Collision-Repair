...
use Bluehost\Plugin\WP\MCP\Domain\Contracts\McpComponentInterface;
use Bluehost\Plugin\WP\MCP\Domain\Prompts\Contracts\McpPromptBuilderInterface;
use Bluehost\Plugin\WP\MCP\Domain\Utils\AbilityArgumentNormalizer;
use Bluehost\Plugin\WP\MCP\Domain\Utils\McpValidator;
...
use Bluehost\Plugin\WP\McpSchema\Server\Prompts\DTO\PromptArgument;
use WP_Error;
/**
...
final class McpPrompt implements McpComponentInterface
{
    ...
    public static function fromArray(array $config)
    {
        ...
        $prompt_data = array('name' => $config['name'], 'description' => $config['description'] ?? null);
        ...
        try {
            ...
            if (isset($config['arguments']) && is_array($config['arguments']) && !empty($config['arguments'])) {
                $prompt_data['arguments'] = array_map(static function (array $arg): PromptArgument {
                    return PromptArgument::fromArray(array('name' => $arg['name'], 'title' => $arg['title'] ?? null, 'description' => $arg['description'] ?? null, 'required' => $arg['required'] ?? null));
                }, $config['arguments']);
            }
            $prompt = PromptDto::fromArray($prompt_data);
        } catch (\Throwable $e) {
            return new WP_Error('mcp_prompt_dto_creation_failed', sprintf(
        ...
        $instance->observability_context = array('component_type' => 'prompt', 'prompt_name' => $config['name'], 'source' => 'array');
        return $instance;
    }
    /**
    ...
    public static function fromAbility(\WP_Ability $ability)
    {
        ...
        $instance->observability_context = array('component_type' => 'prompt', 'prompt_name' => $prompt_data['prompt']->getName(), 'ability_name' => $ability->get_name(), 'source' => 'ability');
        return $instance;
    }
    /**
    ...
    public static function fromBuilder(McpPromptBuilderInterface $builder)
    {
        ...
        $instance->adapter_meta = array('source' => 'builder', 'builder_class' => get_class($builder));
        $instance->observability_context = array('component_type' => 'prompt', 'prompt_name' => $prompt->getName(), 'source' => 'builder');
        return $instance;
    }
    // =========================================================================
    ...
    public function execute($arguments)
    {
        ...
        if (null !== $this->ability) {
            ...
            try {
                ...
                return new WP_Error('mcp_execution_failed', $throwable->getMessage(), array('error_type' => get_class($throwable)));
            }
    ...
    {
        $is_transformed = \true === ($this->adapter_meta['input_schema_transformed'] ?? \false);
        if (!$is_transformed) {
            return $arguments;
        }
    ...
    public function check_permission($arguments)
    {
        ...
        if (null !== $this->ability) {
            ...
            try {
                ...
                return new WP_Error('mcp_permission_check_failed', $throwable->getMessage(), array('error_type' => get_class($throwable)));
            }
        }
        ...
        return new WP_Error('mcp_permission_denied', 'Access denied.', array('failure_reason' => FailureReason::NO_PERMISSION_STRATEGY));
    }
    /**
