...
use Bluehost\Plugin\WP\MCP\Transport\Infrastructure\JsonRpcResponseBuilder;
...
class StdioServerBridge
{
    ...
    public function serve(): void
    {
        ...
        while ($this->is_running) {
            try {
                ...
                $error_response = $this->encode_response(JsonRpcResponseBuilder::create_error_response(null, array('code' => McpErrorFactory::INTERNAL_ERROR, 'message' => 'Internal error', 'data' => array('details' => $e->getMessage()))));
    ...
    private function handle_request(string $json_input): string
    {
        try {
            ...
            $request = json_decode($json_input, \true);
            if (json_last_error() !== \JSON_ERROR_NONE) {
                return $this->create_error_response(null, McpErrorFactory::PARSE_ERROR, 'Parse error', 'Invalid JSON was received by the server.');
            }
            // Validate JSON-RPC structure
            if (!is_array($request)) {
                return $this->create_error_response(null, McpErrorFactory::INVALID_REQUEST, 'Invalid Request', 'The JSON sent is not a valid Request object.');
            }
            // Check for JSON-RPC version
            if (!isset($request['jsonrpc']) || '2.0' !== $request['jsonrpc']) {
                return $this->create_error_response($request['id'] ?? null, McpErrorFactory::INVALID_REQUEST, 'Invalid Request', 'The JSON-RPC version must be 2.0.');
            }
            ...
            $id = $request['id'] ?? null;
            if (!is_string($method)) {
                return $this->create_error_response($id, McpErrorFactory::INVALID_REQUEST, 'Invalid Request', 'Method must be a string.');
            }
            ...
            return $this->create_error_response(null, McpErrorFactory::INTERNAL_ERROR, 'Internal error', $e->getMessage());
        }
    }
    /**
    ...
    private function format_response(array $result, $id): string
    {
        ...
        if (isset($result['error'])) {
            ...
            $error_payload = array('code' => $error['code'] ?? McpErrorFactory::INTERNAL_ERROR, 'message' => $error['message'] ?? 'Internal error');
