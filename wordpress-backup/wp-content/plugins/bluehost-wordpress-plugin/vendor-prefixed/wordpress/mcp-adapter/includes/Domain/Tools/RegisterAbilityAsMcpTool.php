...
use Bluehost\Plugin\WP\MCP\Domain\Utils\McpAnnotationMapper;
use Bluehost\Plugin\WP\MCP\Domain\Utils\McpNameSanitizer;
use Bluehost\Plugin\WP\MCP\Domain\Utils\McpValidator;
use Bluehost\Plugin\WP\MCP\Domain\Utils\SchemaTransformer;
...
class RegisterAbilityAsMcpTool
{
    ...
    public static function build(\WP_Ability $ability)
    {
        ...
        return array('tool' => $tool_dto, 'adapter_meta' => $data['adapter_meta']);
    }
    /**
    ...
    private function build_tool_data()
    {
        ...
        $input_transform = SchemaTransformer::transform_to_object_schema($this->ability->get_input_schema());
        $tool_data = array('name' => $tool_name, 'description' => trim($this->ability->get_description()), 'inputSchema' => $input_transform['schema']);
        ...
        if (!empty($output_schema)) {
            $output_transform = SchemaTransformer::transform_to_object_schema($output_schema, 'result');
        ...
        if (null !== $output_transform && !empty($output_transform['was_transformed'])) {
            $adapter_meta['output_schema_transformed'] = \true;
        ...
        return array('tool_data' => $tool_data, 'adapter_meta' => $adapter_meta);
    }
    /**
