...
use Bluehost\Plugin\WP\MCP\Transport\Contracts\McpTransportInterface;
...
class McpTransportFactory
{
    ...
    public function initialize_transports(array $mcp_transports): void
    {
        foreach ($mcp_transports as $mcp_transport) {
            ...
            if (!class_exists($mcp_transport)) {
                ...
                $this->mcp_server->get_error_handler()->log(sprintf('Transport class "%s" does not exist.', $mcp_transport), array('McpTransportFactory::initialize_transports'));
                continue;
            }
            // Check for interface implementation
            if (!in_array(McpTransportInterface::class, class_implements($mcp_transport) ?: array(), \true)) {
                _doing_it_wrong(__FUNCTION__, sprintf(
                    /* translators: %s: Transport class name */
                ...
                // Log error and continue processing other transports
                $this->mcp_server->get_error_handler()->log(sprintf('MCP transport class "%s" must implement the McpTransportInterface.', $mcp_transport), array('McpTransportFactory::initialize_transports'));
                continue;
            }
    ...
    public function create_transport_context(): McpTransportContext
    {
        ...
        return new McpTransportContext(array('mcp_server' => $this->mcp_server, 'initialize_handler' => $initialize_handler, 'tools_handler' => $tools_handler, 'resources_handler' => $resources_handler, 'prompts_handler' => $prompts_handler, 'system_handler' => $system_handler, 'observability_handler' => $this->mcp_server->get_observability_handler(), 'error_handler' => $this->mcp_server->get_error_handler(), 'transport_permission_callback' => $this->mcp_server->get_transport_permission_callback()));
    }
}
